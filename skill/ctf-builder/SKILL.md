...
3. **Pro mode (optional).** Two-line card pointing at the header toggle. Sentence 1: "If you're already comfortable with this toolset and want to skip the plugin picker, toggle Pro mode in the top-right." Sentence 2 (muted): "You can toggle Pro mode on or off at any stage." Without this card, analysts never discover Pro mode exists (the header toggle is tooltip-only). Keep it terse — explaining WHAT Pro mode does ("the picker is scaffolding for juniors...") reads as condescending; idioms like "not a one-way door" are colloquial and out of place in analyst-facing prose.
