...
3. **Pro mode (optional).** One-sentence card pointing at the header toggle: "If you're already comfortable with this toolset and want to skip the plugin picker, toggle Pro mode in the top-right." Without this, analysts never discover Pro mode exists (the header toggle is tooltip-only). Keep it to one sentence — what Pro mode DOES is self-explanatory once they know it exists; padding it with "the picker is scaffolding for juniors..." reads as condescending.
