...
## The Black Window Case — Memory Forensics Analysis
...
| TA0006 Credential Access | T1003.001 LSASS Memory | HIGH | MEDIUM |
| TA0007 Discovery | T1082 System Information Discovery | LOW | MEDIUM |
| TA0009 Collection | T1560.001 Archive via Utility | HIGH | HIGH |
| TA0010 Exfiltration | T1048 Exfil Over Alternative Protocol (SMB) | MEDIUM | LOW (channel available; not confirmed used) |
...
### 3.5 TA0006 Credential Access — `T1003.001 OS Credential Dumping: LSASS Memory` (working hypothesis)
**Confidence: MEDIUM** | **Severity: HIGH**

Two converging artefacts establish credential-stealing intent (not yet confirmed extraction):

1. **`C:\Windows\debug\PASSWD.LOG`** — A password log file in the Windows debug directory. Normally this directory holds Netlogon debug output (when `HKLM\SYSTEM\CCS\Services\Netlogon\Parameters\DbFlag` is enabled), but is also a well-known dumping ground for credential-stealing tools that emulate legitimate paths. Pivot to disk to confirm contents.
...
WDigest is enabled by default on Win7 SP1, so LSASS holds plaintext credentials in this image — anyone with code execution and the right tools could extract them. Mapping this stage to T1003.001 is a working hypothesis that should be validated by carving `PASSWD.LOG` off disk and inspecting its contents.
...
### 3.7 TA0010 Exfiltration — `T1048 Exfil Over Alternative Protocol` (channel only)
**Confidence: LOW** | **Severity: MEDIUM**

**At capture time, no outbound TCP connections were ESTABLISHED.** Network state shows only loopback, broadcast, and listening sockets — no upload tool (`curl`, `bitsadmin`, PowerShell `Invoke-WebRequest`, etc.) was running.

What IS observed: TCP 445 (SMB) is listening on this host. Combined with the Important.rar staged in Alissa's Documents (§3.6), a credentialed peer could pull the archive off this machine over `\\SMARTNET-PC\C$` with minimal local trace. This is a **viable channel, not confirmed exfil** — the analyst's next pivot is firewall flow logs and disk-side WinRAR archive timestamps to determine whether the file actually left.
...
| 2019-12-11 14:38:00 | Memory image captured (this analysis) | — |
...
| Credential Access | T1003.001 OS Cred Dumping | LSASS Memory | PASSWD.LOG + DPAPI blobs touched | MEDIUM (intent confirmed; extraction not) |
...
| Exfiltration | T1048 Exfil Over Alternative Protocol | SMB | `windows.netscan` — TCP 445 LISTENING; no upload tool observed | LOW (channel available; transfer not confirmed) |
...
# Detect credential-dump / memory-acquisition tools
title: Credential-Dump Tool Execution
logsource: { product: windows, category: process_creation }
detection:
  selection:
    Image|endswith:
